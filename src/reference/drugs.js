...
class Drug_Weight extends Drug {
    ...
    dose(weight) {
        if (Array.isArray(this._dose)) { //if there is a list of doses then calculate the dose range
            return `<small>${this._dose[0]} - ${this._dose[1]}</small> <br>
            ${math.format(math.eval(this._dose[0] + "*" + weight + "kg"), 3)} - ${math.format(math.eval(this._dose[1] + "*" + weight + "kg"), 3)}`
        } else { //otherwise just calculate the dose
            return `<small>${this._dose}</small> <br>
            ${math.format(math.eval(this._dose + "*" + weight + "kg"), 3)}`
        }
    }
    ...
    presentation(weight) {
        if (Array.isArray(this._dose)) { //if there is a list of doses then calculate the dose range
            return `<small>${this._presentation}</small> <br>
                ${ math.format(math.eval("(" + this._dose[0] + "*" + weight + "kg) / (" + this._presentation + ")"), 3)} - 
                ${math.format(math.eval("(" + this._dose[1] + "*" + weight + "kg) / (" + this._presentation + ")"), 3)}  `
        } else { //otherwise just calculate the dose
            return `<small>${this._presentation}</small> <br>
                ${ math.format(math.eval("(" + this._dose + "*" + weight + "kg) / (" + this._presentation + ")"), 3)}  `
        }
    }
